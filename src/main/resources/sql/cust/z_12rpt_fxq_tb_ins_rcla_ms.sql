...
alter table rpt_fxq_tb_ins_rcla_ms add partition (partition pt20190827000000 values less than ('{lastday}999999'));

alter table rpt_fxq_tb_ins_rcla_ms truncate partition pt20190827000000;
*/
...
from ods_cthx_web_ply_base partition(pt20190827000000) a
	 ...
	 inner join ods_cthx_web_app_insured partition(pt20190827000000) c on a.c_app_no=c.c_app_no
     inner join ods_cthx_web_ply_bnfc partition(pt20190827000000) d on  a.c_ply_no=d.c_ply_no -- 多个受益人相应生成多条记录
	 -- inner join ods_cthx_web_fin_clm_due partition(pt20190827000000) u on a.c_ply_no = u.c_ply_no and u.c_feetyp_cde ='CPPK'
	 inner join ods_cthx_web_clmnv_endcase partition(pt20190827000000) u on a.c_ply_no = u.c_clm_no and u.c_feetyp_cde ='CPPK'
	 inner join ods_cthx_web_clm_bank partition(pt20190827000000) g on u.c_clm_no=g.c_clm_no
	 inner join ods_cthx_web_clm_rpt partition(pt20190827000000) e on g.c_clm_no=e.c_clm_no
