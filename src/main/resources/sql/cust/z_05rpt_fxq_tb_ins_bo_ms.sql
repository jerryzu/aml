...
alter table rpt_fxq_tb_ins_bo_ms add partition (partition pt20190903000000 values less than ('{lastday}999999'));

alter table rpt_fxq_tb_ins_bo_ms truncate partition pt20190903000000;
*/
/**
本段取法人投保，团单受益人是个人的单子
**/
...
select 
    ...
    '@N'as sys_name,-- 系统名称,
    '20190903' pt
from edw_cust_ply_party partition(pt20190903000000) a1
    inner join edw_cust_ply_party partition(pt20190903000000) a2 on a1.c_ply_no = a2.c_ply_no
    left join  edw_cust_units_info partition(pt20190903000000)  u on a1.c_cst_no = u.c_cst_no
    left join edw_cust_pers_info partition(pt20190903000000) p on a2.c_cst_no = p.c_cst_no
