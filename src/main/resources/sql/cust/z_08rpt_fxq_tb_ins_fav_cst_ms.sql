...
alter table rpt_fxq_tb_ins_fav_cst_ms add partition (partition pt20190903000000 values less than ('{lastday}999999'));

alter table rpt_fxq_tb_ins_fav_cst_ms truncate partition pt20190903000000;
*/
...
from ods_cthx_web_ply_base partition(pt20190903000000) a
	left join edw_cust_ply_party_applicant partition(pt20190903000000) b on a.c_app_no=b.c_app_no
	left join edw_cust_ply_party_bnfc partition(pt20190903000000) c on a.c_app_no=c.c_app_no
where a.t_next_edr_bgn_tm > now() and b.c_clnt_mrk = 1