...
public class CompanyDAO {
    public static void ConvertData(QueryRunner runner, QueryRunner targetqr) {
        try {
            ...
            String inssql = "INSERT INTO tb_company(head_no,company_codel,company_code2,company_name,bord_flag)VALUES(?,?,?,?,?)";
            List<Company> companys = runner.query(sql, new BeanListHandler<Company>(Company.class));
            for (Company company : companys) {
                Object[] params = { company.getHead_no(), company.getCompany_codel(), company.getCompany_code2(),
