...
public class InsRchgDAO {
    ...
    public static void ConvertData(QueryRunner runner, QueryRunner targetqr) {
        try {
            ...
            String inssql = "INSERT INTO tb_ins_rchg(company_codel, company_code2, company_code3, company_code4, pol_no, app_no, app_name, app_cst_no, app_date, chg_date, chg_no, item, con_bef)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            List<InsRchg> insRchgs = runner.query(sql, new BeanListHandler<InsRchg>(InsRchg.class));
            for (InsRchg insRchg : insRchgs) {
                Object[] params = { insRchg.getCompany_codel(), insRchg.getCompany_code2(), insRchg.getCompany_code3(),
